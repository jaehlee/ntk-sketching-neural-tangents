...
def MyrtleNetworkFeatures(depth, W_std=np.sqrt(2.0), b_std=0., **relu_args):
  ...
  layers = []
  layers += [conv_fn(width, filter_size=3), ReluFeatures(**relu_args)] * layer_factor[depth][0]
  layers += [AvgPoolFeatures(2, 2)]
  layers += [ConvFeatures(width, filter_size=3, W_std=W_std), ReluFeatures(**relu_args)] * layer_factor[depth][1]
  layers += [AvgPoolFeatures(2, 2)]
  layers += [ConvFeatures(width, filter_size=3, W_std=W_std), ReluFeatures(**relu_args)] * layer_factor[depth][2]
